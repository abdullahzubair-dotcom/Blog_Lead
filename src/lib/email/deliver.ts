...
export async function deliverOutreach(opts: {
  ...
  if (opts.sender) {
    ...
    return sendEmailAs({ user: opts.sender, pass, fromName: cfg.from_name, to: opts.to, subject: opts.subject, body: opts.body, cc });
  }
  return sendEmail({ to: opts.to, subject: opts.subject, body: opts.body, cc });
}
